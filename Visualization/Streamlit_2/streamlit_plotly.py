...
clusters = st.sidebar.slider(min_value=1, max_value=6, value=3, label="Select a number of clusters")
...
cols.remove("Species")
fig_box_2 = px.scatter_matrix(df, dimensions=cols, color="Species", color_discrete_map=colors)
...
fig_box_4 = px.line(x=x, y=inertias, labels={
    "x":"Number of Clusters",
    "y":"Innertia"
})
fig_box_4.update_layout(height=700)
...
fig_predict = px.scatter(df, x="petal width (cm)", y="petal length (cm)", color="Cluster")
fig_actual =  px.scatter(df, x="petal width (cm)", y="petal length (cm)", color="Species",color_discrete_map=colors)
with col1:
