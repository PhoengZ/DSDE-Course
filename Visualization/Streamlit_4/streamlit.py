...
clusters = st.sidebar.slider(min_value=1, max_value=6, value=3, label='Number of Clusters')
...
fig_graph = px.scatter_matrix(df, dimensions=feature_names, color='Species', color_discrete_map=colors, title='Feature Relationships By Species', category_orders={'Species': sorted(df['Species'].unique())}, height=600)
...
fig_elbow = px.line(df, x=[i for i in range(1,11)], y=inertias, labels={"x":"Number of Clusters", "y":"Inertias"}, title="Elbow Method Analysis")
_, col, _ = st.columns([1,3,1])
with col:
...
with col1:
    ...
    fig_cluster = px.scatter(df, x='petal length (cm)', y='petal width (cm)',color="Cluster", title="Kmeans Clustering Result")
    st.plotly_chart(fig_cluster)

with col2:
    st.subheader('Actual Species')
    fig_cluster = px.scatter(df, x='petal length (cm)', y='petal width (cm)',color="Species", color_discrete_map=colors, title="Actual Species")
    st.plotly_chart(fig_cluster)
