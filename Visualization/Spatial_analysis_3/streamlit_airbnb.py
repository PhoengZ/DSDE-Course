...
eps_degrees = st.sidebar.slider(min_value=0.001, max_value=0.005, value=0.002, step=0.001, format="%.3f", label="eps_degrees")
                              
min_samples = st.sidebar.slider(min_value=2, max_value=10, value=3, label="min_samples")                             
...
try:
    ...
    st.pydeck_chart(pdk.Deck(
        layers=[heatmapLayer], initial_view_state=heatmap_init
    ))    
    # Create and display the map for heatmap layer


    hexagonalLayer = pdk.Layer(
        "HexagonLayer",
        viz_data,
        get_position=['longitude', 'latitude'],
        opacity=0.6
    )
    hexagonal_init = pdk.ViewState(
        longitude=viz_data["longitude"].mean(),
        latitude=viz_data['latitude'].mean(),
        zoom=11,
        pitch=0
    ...
    st.pydeck_chart(pdk.Deck(
        layers=[hexagonalLayer], initial_view_state=hexagonal_init
    ))
    ...
    with st.expander("View Cluster Statistics", expanded=False):
        for cluster_id in top_clusters.index:
            ...
            st.write("**Neighborhoods:**", ', '.join(cluster_data['neighbourhood'].unique()[:10]))
            ...
            sample_listings = cluster_data[['name', 'neighbourhood', 'price', 'number_of_reviews']].head(5)
...
try:
    ...
    kde = KernelDensity(bandwidth=bandwidth) # Add correct parameters
    kde.fit(coords)
    ...
    st.pydeck_chart(pdk.Deck(
        layers=[scatterLayer], initial_view_state=init_scatter,
            tooltip={
                "html":
                       "<b>Price:</b> ฿{price}<br/>"
                       "<b>Name:</b> {name}<br/>"
                        "<b>Density normallized:</b> {density_normalized}<br/>"
